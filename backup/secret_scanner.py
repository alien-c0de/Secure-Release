...
# -----------------------------------------
# 🔥 Severity Mapping for Detect-Secrets
# -----------------------------------------
...
def scan_with_detect_secrets(file_path: Path):
    ...
    with default_settings():
        ...
        for secret in secrets:
            plugin_name = getattr(secret, "type", "Unknown")

            findings.append({
                "severity": get_severity(plugin_name),
                "plugin": plugin_name,
                "file": str(file_path),
...
async def scan(config):
    ...
    # print(Fore.BLUE + Style.BRIGHT + f"[+] 📢 Total Secrets Found: {len(combined)} (Detect-Secrets + TruffleHog3)" + Style.RESET_ALL)
    print(Fore.BLUE + Style.BRIGHT + f"[+] 📢 Secret scan found" + Fore.WHITE + Style.BRIGHT, len(combined), Fore.BLUE + Style.BRIGHT + f"issues.", Fore.RESET)
