...
async def scan_single_file(req_file):
    ...
    try:
        output = await run_subprocess_py(
            f"safety scan --output json --file={req_file}",
            cfg.get("target_dirs", ["./"])[0]
        )
                ...
                for dependency in file_entry.get("results", {}).get("dependencies", []):
                    ...
                    for spec in dependency.get("specifications", []):
                        ...
                        for vuln in vulnerabilities:
                            ...
                            # severity structure:
                            # { "level": "high", "score": 7.5 }
                            severity_level = severity_info.get("level", "UNKNOWN").upper()

                            # For description we use ignored.reason (why?)
...
async def run_subprocess_py(command, cwd=None, timeout=6000):
    try:
